...
def process_cctv_htmls_with_title(html):
    if not html:
        return None
    title_text = None
    try:
...
def process_cctv_htmls_with_info(html):
    if not html:
        return None
    info_text = None
    try:
...
def process_cctv_htmls_with_content(html):
    if not html:
        return None
    content_text = None
...
def stoarge_cctv_htmls(news):

    return None


def get_cctv_news(website_name, url):
    news_links = scrape_cctv_links(url)
    for link in news_links:
        news_html = scrape_cctv_html(link)

        title = process_cctv_htmls_with_title(news_html)
        info = process_cctv_htmls_with_info(news_html)
        content_origin = process_cctv_htmls_with_content(news_html)
